      ...
      <div style={{ maxWidth: '1280px', margin: '0 auto', padding: '1rem' }}>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(280px, 1fr))', gap: '1rem', marginBottom: '2rem' }}>
          <div>
            <div style={{ background: '#fff', borderRadius: '16px', overflow: 'hidden', border: '1px solid #f3f4f6', marginBottom: '10px' }}>
              {product.images?.[selectedImage]?.url ? (
                <img src={product.images[selectedImage].url} alt={product.name} style={{ width: '100%', aspectRatio: '1', objectFit: 'cover' }} />
              ) : (
          ...
          <div style={{ background: '#fff', borderRadius: '16px', padding: '1.25rem', border: '1px solid #f3f4f6' }}>
            ...
            <h1 style={{ fontSize: 'clamp(1.1rem, 4vw, 1.5rem)', fontWeight: '700', color: '#111827', marginBottom: '10px', lineHeight: 1.3 }}>{product.name}</h1>
            ...
            <div style={{ display: 'flex', alignItems: 'center', gap: '12px', marginBottom: '16px' }}>
              <div style={{ display: 'flex', alignItems: 'center', border: '1px solid #e5e7eb', borderRadius: '10px', overflow: 'hidden' }}>
                <button onClick={() => setQuantity(q => Math.max(1, q - 1))}
                  style={{ width: '36px', height: '36px', background: '#fff', border: 'none', cursor: 'pointer', fontSize: '18px', fontWeight: '700', color: '#374151' }}>-</button>
            ...
            <div style={{ display: 'flex', gap: '10px', marginBottom: '16px' }}>
              ...
              <Link href="/checkout" onClick={handleAddToCart}
                style={{ flex: 1, background: '#111827', color: '#fff', borderRadius: '12px', padding: '14px', fontSize: '15px', fontWeight: '700', textDecoration: 'none', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
                Buy Now
              </Link>
            </div>
